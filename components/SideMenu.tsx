...
export const SideMenu: React.FC<SideMenuProps> = ({
  ...
  useEffect(() => {
    if (activeAccordion && scrollContainerRef.current) {
      const accordionElement = scrollContainerRef.current.querySelector(
        `[data-accordion-id="${activeAccordion}"]`,
      );
      if (accordionElement) {
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed top-0 left-0 h-full w-[90vw] sm:w-full max-w-md bg-slate-900 border-r border-white/10 shadow-2xl z-50 flex flex-col"
            role="dialog"
            aria-modal="true"
          >
